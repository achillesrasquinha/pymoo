...
class ZDT1WithGradient(ZDT):
    ...
    def _evaluate(self, x, out, *args, **kwargs):
        ...
        if "dF" in out:
            dF = jnp.zeros([x.shape[0], self.n_obj, self.n_var], dtype=float)
            dF[:, 0, 0], dF[:, 0, 1:] = 1, 0
            dF[:, 1, 0] = -0.5 * jnp.sqrt(g / x[:, 0])
            dF[:, 1, 1:] = ((9 / (self.n_var - 1)) * (1 - 0.5 * jnp.sqrt(x[:, 0] / g)))[:, None]
            out["dF"] = dF
...
if __name__ == '__main__':
    ...
    X = jnp.random.random((10, problem.n_var))
